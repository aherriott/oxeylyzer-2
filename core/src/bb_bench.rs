...
mod tests {
    ...
    fn bench_bb_node_simulation() {
        ...
        println!("  Score with 5 keys: {}", cache.score());
        ...
        println!("  {} positions x {n} iterations in {:?}", positions_tried, elapsed);
    ...
    fn bench_greedy_completion() {
        ...
        let remaining: Vec<usize> = (5..num_positions)
            ...
            .collect();
        let available: Vec<usize> = (5..num_positions).collect();

        println!("\n=== Greedy completion cost ===");
        let n = 10u32;
        let t = Instant::now();
        let mut score = 0i64;
        for _ in 0..n {
            score = cache.greedy_completion_score(&remaining, &available);
        }
        println!("  {:?}/call ({n} iterations), score={}", t.elapsed() / n, score);
    }
}
